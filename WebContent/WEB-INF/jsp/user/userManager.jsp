								...
								<c:forEach var="ua" items="${userArticle }">
								  <tr>
								  	<td>
									  	...
									  	<div style="float: right;">
										  	<fmt:formatDate value="${ua.date }" pattern="yyyy/MM/dd HH:mm:ss" />
										  	<button type="button" class="btn btn-danger" onclick="adminDeleteArticle(${ua.aid })">删除</button>
									  	</div>
									</td>
								  </tr>
								</c:forEach>
...
<script type="text/javascript">
	...
	function deleteArticle(aid){
		$.get("${pageContext.request.contextPath}/article/delete/"+aid,function(data){
			alert(data.data);
		location.href = "${pageContext.request.contextPath}/user/manager/${u.uid }";},"json");
	}
	
	/*管理员删除帖子*/
	function adminDeleteArticle(aid){
